...
from nemo_aligner.utils.deep_search.mcts.mcts import DeepSearch, ParallelSearch, MCTSParallel
...
offset = 1765
ps = [ParallelSearch(tokenizer.encode(steerlm_template.format(prompt=score_fun.ds['train'][i]['question'])),  i) for i in range(offset, offset+args['rollout_micro_batch_size'])]
...
def get_client_fun():
    def trion_infer_batch(sentences=None, action=None, context_ids=None, session_info=None):
        ...
        if sentences is not None:
            ...
            result_dict = client.infer_batch(sentences=input_data, context_ids=context_ids,  parameters={"session": session_info})
        else:
            result_dict = client.infer_batch(action=action, context_ids=context_ids, parameters={"session": session_info})
        return result_dict
    return trion_infer_batch

mcts = MCTSParallel(args, tokenizer, session_info='test_selfplay', score_fn=score_fun, terminate_fns=[termination_condition], client_fun=get_client_fun())

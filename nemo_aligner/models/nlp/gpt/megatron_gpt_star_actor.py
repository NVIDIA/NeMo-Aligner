...
class MegatronGPTSTaRModel(NLPAdapterModelMixin, MegatronGPTModel, AlignableGenerativeInterface):
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        self._length_params = OmegaConf.to_container(self.cfg.star.length_params, resolve=True)
        # sampling parameters for generation
        self._sampling_params = OmegaConf.to_container(self.cfg.star.sampling_params, resolve=True)

        self.to_offload_adam_states = self.cfg.star.offload_adam_states
        self.forward_micro_batch_size = self.cfg.star.forward_micro_batch_size
    ...
    def get_actor_forward_output_and_loss_func(self):
        def fwd_output_and_loss_func(data_iterator, model):
            ...
            mask = batch["mask"]
            weights = batch.get("weights", torch.ones(response_tokens.shape[0])).cuda()
            print(weights)
            ...
            def loss_func(parallel_logits):
                ...
                loss = -1 * masked_mean(curr_log_probs * weights, mask)  # Loss is mean logits on response tokens

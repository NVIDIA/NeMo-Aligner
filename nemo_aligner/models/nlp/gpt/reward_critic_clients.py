...
def extract_all_dialog(text):
    pattern_system = re.compile(r"<\|start_header_id\|>system<\|end_header_id\|>\n\n(.*?)<\|eot_id\|>", re.DOTALL)
    pattern_user = re.compile(r"<\|start_header_id\|>user<\|end_header_id\|>\n\n(.*?)<\|eot_id\|>", re.DOTALL)
    pattern_assistant = re.compile(
        r"<\|start_header_id\|>assistant<\|end_header_id\|>\n\n(.*?)<\|eot_id\|>", re.DOTALL
    )

    system_dialogues = pattern_system.findall(text)
    user_dialogues = pattern_user.findall(text)
    assistant_dialogues = pattern_assistant.findall(text)

    return system_dialogues, user_dialogues, assistant_dialogues
...
class RemoteGPTRMCriticClient:
    ...
    def infer_rm(self, inputs):
        ...
        for (text, is_end) in inputs:
            ...
            _, user_text, assistant_text = extract_all_dialog(text)

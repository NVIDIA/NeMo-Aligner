...
class MegatronNevaActorModel(MegatronNevaModel, AlignableGenerativeInterface):
    def __init__(self, cfg: DictConfig, trainer: Trainer):
        ...
        self._length_params = length_params #OmegaConf.to_container(self.cfg.ppo.length_params, resolve=True)
        # sampling parameters for generation
        self._sampling_params = sampling_params #OmegaConf.to_container(self.cfg.ppo.sampling_params, resolve=True)
    ...
    def get_logprob_output_only_func(self, inference_only=True):
        ...
        def log_prob_output_only_func(dataloader_iter, model):
            ...
            # print("PRINT 2 batch = ", batch[0])
            print("PRINT 2 batch[2] shape= ", batch[2].shape)
            batch[0] = batch[0][:,:330]
            batch[2] = batch[2][:,:330]
            output_tensor, _ = fwd_output_only_func(iter([batch,]), model)
            # print("Original output shape = ", output_tensor1.shape)
            '''batch_ = [batch[0][:,:325], batch[1], batch[2][:,:325], batch[3], batch[4], batch[5]]
            output_tensor, _ = fwd_output_only_func(iter([batch_,]), model)
            print(f"ii = 0, output sum = {output_tensor.sum()}, batch0 = {batch_[0]}, batch4 = {batch_[4]}, batch5 = {batch_[5]}, part2 = {batch_[0].shape}")
            all_outs = [output_tensor]
            for ii in range(327-325):
                ii = ii + 325
                batch_ = [batch[0][:,ii:ii+1], batch[1], batch[2][:,ii:ii+1], batch[3], batch[4], batch[5]]
                batch_[4] = torch.tensor([False], device='cuda:0')
                output_tensor, _ = fwd_output_only_func(iter([batch_,]), model)
                if ii == 325:
                    print(f"ii = {ii-325+1}, batch0 = {batch_[0]}, batch5 = {batch_[5]}, output sum = {output_tensor.sum()}, part2 = {batch_[0].shape}") 
                all_outs.append(output_tensor)
  
            output_tensor = torch.cat(all_outs, dim=1)
            print(f"output_tensor = {output_tensor.shape}")
            import torch.nn.functional as F
            tokens = batch[0]
            context_length = 326
            output = F.log_softmax(output_tensor[:, :context_length, :], 2)
            indices = torch.unsqueeze(tokens[:, 1 : context_length + 1], 2)
            # print(tokens[:, 1 : context_length + 1])
            # print(tokens[:, 0 : context_length + 1])
            output_tensor = torch.gather(output, 2, indices).squeeze(2), output_tensor'''
            # print("indices = ", indices)
            
            # for i in range(output_tensor[:,:,:].shape[1]):
            #     print(f"PRINT 2 , i = {i} {output_tensor[:,i,:].shape, output_tensor[:,i,:].sum()}")
            def id_func(output_tensor, non_loss_data=True):
                # log_prob_before, output_tensor = output_tensor
                ...
                # output_tensor = log_prob_before
                return logprobs#output_tensor
    ...
    @torch.no_grad()
    def get_inference_log_probs(self, media, response_tokens, forward_micro_batch_size):
        ...
        set_inference_key_value_memory, inference_max_sequence_len = torch.tensor([True]).to('cuda'), torch.tensor([attention_mask.shape[-1]]).to('cuda')

        batch_iter = get_iterator_k_split([response_tokens, attention_mask, position_ids,
                                           media, set_inference_key_value_memory, inference_max_sequence_len], num_microbatches)
        
        print("*"*100)
        print("*"*100)
        print("*"*100)
        print("MOVING toward 2")
        print("*"*100)
        print("*"*100)
        print("*"*100)
        fwd_bwd_function = get_forward_backward_func()
    ...
    def infer(self, inference_batch):
        ...
        for i in range(inference_batch["image"].shape[0]):
            ...
            context_tokens_tensor, context_length_tensor = neva_tokenize_batch(
                        inference_batch["prompt"][i], self._length_params['max_length'], self._sampling_params['add_BOS'],
                        self.tokenizer, self.cfg)
            list_token_tensors.append(context_tokens_tensor)
            list_length_tensor.append(context_length_tensor)

        

        actor_output = self.generate(
        input_prompts=final_prompts, length_params=self._length_params, sampling_params=self._sampling_params, inference_config=self.cfg
        )


        response_tokens = torch.stack([torch.cuda.LongTensor(actor_output[i]["token_ids"]).flatten() for i in range(len(actor_output))])
        prompt_lengths = torch.stack(list_length_tensor)
        ...
        # TODO (ataghibakhsh): check how log_probs calculation has
        #        changed (added to the config to calculate logprobs)

        log_probs1 = actor_output[0]["logprob"]
        # log_probs1 = self.get_inference_log_probs(
        #     None, response_tokens, forward_micro_batch_size= self.forward_micro_batch_size)
        log_probs2 = self.get_inference_log_probs(
            final_prompts[0]['image'], response_tokens, forward_micro_batch_size= self.forward_micro_batch_size
        )
        print("**************************")
        print(f"log_probs1 shape = {log_probs1.shape}")
        print(f"log_probs2 shape = {log_probs2.shape}")
        print(f"diff relative log-prob = {(log_probs1 - log_probs2)/log_probs2}")
        print(f"diff log-prob = {(log_probs1 - log_probs2)}")
        import sys
        sys.exit()
        rollout_batch = {
            "response_tokens": response_tokens,
            "response_lengths": response_lengths, # prompt + response num tokens 
            "prompt_lengths": prompt_lengths, # 
            "logprobs": log_probs,
        }

    ...
    def annealed_guidance(self, batch, x_T, weighing_fn=None):
        ...
        with torch.cuda.amp.autocast(
            ...
            for i in iterator:
                ...
                x_next_draft, eps_draft = sampler.sampler_step(s_in * sigmas[i], s_in * sigmas[i+1], denoiser_draft, x.clone(), cond, uc, gamma, return_noise=True)
                # get base model 
                with adapter_control(base_model):
                    _, eps_init = sampler.sampler_step(s_in * sigmas[i], s_in * sigmas[i+1], denoiser_base, x.clone(), cond, uc, gamma, return_noise=True)
                # get weighing scheme
                ...
                # get next x
                x = sampler.possible_correction_step(euler_step, x.clone(), eps, dt, s_in * sigmas[i+1], denoiser_draft, cond, uc)

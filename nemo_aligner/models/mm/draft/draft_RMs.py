...
class PickscoreRewardModel(MegatronModule):
    ...
    def get_reward(self, images, captions):
        ...
        rewards = self.logit_scale.exp() * torch.matmul(F.normalize(image_features, dim=-1), F.normalize(text_features, dim=-1).t()).diag()
...
class MegatronCLIPRewardModel(MegatronCLIPModel):
    ...
    def preprocess(self, images, captions):
        ...
        images = torch.stack([self.differentiable_preprocess(img.permute(2, 0, 1)) for img in images]).to(torch.cuda.current_device()).float()

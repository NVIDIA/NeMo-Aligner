...
from nemo_aligner.experimental.run.gpt_dpo import default_dpo_data_config
...
def experiment(
    ...
    with run.Experiment("dpo_experiment", executor=executor, log_level="WARN") as exp:
        ...
        dpo_loop_fn = run.Partial(
            dpo_loop, restore_from_path=restore_from_path, data_config=data_config, tp=tp, pp=pp, vp=vp
        )

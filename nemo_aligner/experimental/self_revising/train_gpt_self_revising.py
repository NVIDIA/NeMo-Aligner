...
"""Script to start Self-Revising training"""
...
def main(cfg) -> None:
    ...
    if cfg.model.data.get("sample", False):
        ...
        if cfg.trainer.self_revising.max_steps < 0:
            num_samples = None
        else:
    ...
    if cfg.model.data.get("sample", False):
        num_samples = cfg.trainer.self_revising.limit_val_batches * val_data_cfg.global_batch_size
    else:
        num_samples = None
    ...
    self_revising_trainer = SelfRevisingTrainer(
        cfg=cfg.trainer.self_revising,
        model=ptl_model,
        optimizer=optimizer,
        scheduler=scheduler,
    ...
    if custom_trainer_state_dict is not None:
        self_revising_trainer.load_state_dict(custom_trainer_state_dict)

    self_revising_trainer.fit()

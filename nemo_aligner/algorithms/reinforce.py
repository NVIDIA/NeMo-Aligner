...
class ReinforceTrainer:
    ...
    def compute_rollout_metrics(self, rollout_batch, save_val_responses=False):
        ...
        metrics = {
            ...
            "percentage_of_samples_with_reward_gap_gt_0": sum([1 for gap in reward_gaps if gap > 0])
            / len(reward_gaps),
            "percentage_of_samples_with_reward_gap_gt_0": sum([1 for gap in reward_gaps if gap > 0])
            / len(reward_gaps),
            "percentage_of_nonperfect_samples_with_reward_gap_gt_0": sum(
                [1 for gap in reward_gaps_for_nonperfect_sample if gap > 0]
            )
            / len(reward_gaps_for_nonperfect_sample)
            if len(reward_gaps_for_nonperfect_sample) > 0
            else -100,
        }

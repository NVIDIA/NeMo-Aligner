...
from nemo_aligner.utils.ppo_utils import create_mask, select_topk, weight_by_correect
...
class StarTrainer:
    """Trainer to coordinate Star training
    """
    ...
    def generate_rs_data(self, rollout_batches):
        ...
        for rollout_batch in rollout_batches:
            ...
            rs_rollout_data["weights"].extend(post_process_tensor(rollout_batch["weights"]))
        ...
        if not is_validation:
            ...
            for _, inference_batch in zip(range(num_microbatches), dataloader_iter):
                ...
                for _ in range(self.num_rollouts_per_prompt):
                    ...
                    reward = self.rm.infer_rm(rollout_batch).result().detach().flatten()

                    prompt_tokens.append(inference_batch["problem"])
                    response_tokens.append(rollout_batch["response_tokens"])
                    response_lengths.append(rollout_batch["response_lengths"])
                    response_sentences.append(rollout_batch["response_sentences"])
                    prompt_lengths.append(rollout_batch["prompt_lengths"])
                    rewards.append(reward)
                    ground_truths.append(rollout_batch["ground_truths"])
                ...
                all_rollouts["ground_truths"] = [item for sublist in ground_truths for item in sublist]

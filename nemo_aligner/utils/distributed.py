...
def _compute_distributed_top_k_p(logits, k, p, rank, world_size):
    ...
    if rank == 0:  # only rank 0 will do the computation and scatter the outcome
        ...
        global_top_k_p_indices = torch.empty_like(
            local_topk_indices, dtype=local_topk_indices.dtype, device=local_topk_indices.device
        )  # [B,S,k]
    ...
    full_mask[valid_local_batch_idx, valid_local_sequence_idx, valid_local_vocab_idx] = logits[
        valid_local_batch_idx, valid_local_sequence_idx, valid_local_vocab_idx
    ]
    return full_mask
        ...
        if sampling_params is not None:
            if prompt_lengths is None:
                prompt_lengths = torch.zeros(
                    vocab_parallel_logits.size(0), device=vocab_parallel_logits.device, dtype=torch.int32
                )

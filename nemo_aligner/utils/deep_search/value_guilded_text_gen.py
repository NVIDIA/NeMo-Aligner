...
import copy
from test_reward_client import process_sample, get_reward

template = Template('''<extra_id_0>System
...
tokenizer = sentencepiece.SentencePieceProcessor('/datasets/models/unpack_10b_solar_steerlm/0c96894aab214922922f717b00c1a8e4_solar_tokenizer.model')
...
record0 = {'conversations': [{'from': 'User', 'value': 'how are you?'}, {'from': 'Assistant', 'value': 'I am fine, thank you.'}, {'from': 'User', 'value': 'why the sky is blue?'}, {'from': 'Assistant', 'value': ''}]}
record1 = {'conversations': [{'from': 'User', 'value': 'how are you?'}, {'from': 'Assistant', 'value': 'I am fine, thank you.'}, {'from': 'User', 'value': 'why the ocean is blue?'}, {'from': 'Assistant', 'value': ''}]}
...
with ModelClient("localhost:2323", "search") as client:
    result_dict = client.infer_batch(sentences=input1_data, context_ids=encoded_context_data, parameters={"session": "test"})
...
for i in range(max_tokens):
    ...
    # 
    with ModelClient("localhost:2323", "search") as client:
        result_dict = client.infer_batch(action=np.array(next_actions).reshape(len(next_actions), -1).astype(np.int32), context_ids=encoded_context_data, parameters={"session": "test"})
        result_dict = adjust_actions(result_dict, threshold=threshold)

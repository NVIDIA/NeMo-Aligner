#!/bin/bash
#SBATCH -N 1 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:joint_training_actor -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 4 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:joint_training_rm -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
NAME="llama3-8b-lr3e-7-bsz512-kl0.05-test-4node-joint-training"
COMMIT_ID=a4b5302
...
TRAIN_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama3_combined_train_data.jsonl"
VALID_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama3_combined_val_data.jsonl"
...
&& python -u examples/nlp/gpt/train_gpt_reinforce_ifeval.py \
    ...
    exp_manager.checkpoint_callback_params.save_top_k=8 \
    ...
    trainer.reinforce.save_interval=50 \
    trainer.num_nodes=4 \
    trainer.devices=8 \
    ++model.tensor_model_parallel_size=4 \

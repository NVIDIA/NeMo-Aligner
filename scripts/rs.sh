#!/bin/bash
#SBATCH -N 8 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rs-bo8-rlfh:42-9_critic+reward -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
#SBATCH hetjob
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rs-bo8:42-9_actor+init_policy -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
...
NAME="ablation-eloirm-rs-bo8-eloi-ct2-hh-lr1e-6-128-fmbs8"
COMMIT_ID=eb86c49
...
pushd ${NEMO_RLHF_DIR}
git fetch origin
git checkout -B rejection_sampling_clean ${COMMIT_ID} || exit 1
popd
popd

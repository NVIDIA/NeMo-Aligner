#!/bin/bash
#SBATCH -N 8 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_grpo:42-9_critic+reward -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
#SBATCH hetjob
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_grpo:42-9_actor+init_policy -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
...
NAME="ablation-eloirm-grpo-hh-lr1e-6-bsz64-kl0.01-fmbs8"
COMMIT_ID=770873d
...
&& export PYTRITON_HOME=/pytriton_cache \
&& export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512 \
&& python -u examples/nlp/gpt/train_gpt_grpo_actor.py \
    ...
    model.ppo.forward_micro_batch_size=8 \
    model.ppo.val_rollout_micro_batch_size=8 \

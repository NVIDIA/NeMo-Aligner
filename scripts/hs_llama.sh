#!/bin/bash
#SBATCH -N 1 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:llama_training_actor -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 4 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:llama_training_rm -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
NAME="llama3.1-8b-RLOO-lr1e-6-kl0.05-helpsteer"
COMMIT_ID=3c14633
...
CONTAINER_ACTOR="gitlab-master.nvidia.com/dl/joc/nemo-ci/main/train:pipe.16440368-x86"
...
pushd ${NEMO_RLHF_DIR}
git fetch origin
git checkout -B llama3.1 ${COMMIT_ID} || exit 1
popd
popd
...
ACTOR_NEMO_FILE="/lustre/fs3/portfolios/llmservice/projects/llmservice_modelalignment_sft/rlhf/checkpoints/community/llama3.1/8b_instruct"
DATASET_DIR="${RLHF_SHARED_DIR}/data/extra_id_prefix_end_with_backslash_n_extra_id_1_jsonl"
TRAIN_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama_helpsteer_train.jsonl"
VALID_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama_helpsteer_val.jsonl"
...
&& python -u examples/nlp/gpt/train_gpt_reinforce_ifeval.py \
    ...
    model.reinforce.num_val_samples=128 \

#!/bin/bash
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo:llama_reward -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 4 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo:llama_actor -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
NAME="llama3-8b-lr3e-7-bsz64-kl0.01-test-4node"
COMMIT_ID=4edc551
...
pushd ${NEMO_RLHF_DIR}
git fetch origin
git checkout -B reinforce ${COMMIT_ID} || exit 1
popd
popd
...
RM_NEMO_FILE="/lustre/fsw/portfolios/llmservice/users/shengyangs/results/projects/nemotron-synthetic-data/preference_optimization/data/preference/blends/v1_opal-rattlesnake.jsonl"
...
TRAIN_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama3_train_data.jsonl"
VALID_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/llama3_val_data.jsonl"
...
&& export HF_HOME="/lustre/fsw/portfolios/llmservice/projects/llmservice_modelalignment_sft/rlhf/checkpoints/community/llama3/hf_home" \

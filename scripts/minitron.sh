#!/bin/bash
#SBATCH -N 1 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:minitron2_training_actor -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 4 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo_llama:minitron2_training_rm -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
TRAIN_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/minitron_helpsteer_train.jsonl"
VALID_DATA_PATH="/lustre/fsw/portfolios/llmservice/users/abukharin/data/minitron_helpsteer_val.jsonl"

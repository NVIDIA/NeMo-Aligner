#!/bin/bash
#SBATCH -N 16 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo:42-9_critic+reward -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_rloo:42-9_actor+init_policy -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
NAME="ablation-eloirm-rloo-hh-lr1e-7-bsz512-kl0.01-large"
COMMIT_ID=770873d

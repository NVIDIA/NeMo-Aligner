#!/bin/bash
#SBATCH -N 8 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_remax:42-9_critic+reward -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
#SBATCH hetjob
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_remax:42-9_actor+init_policy -t 4:00:00 --dependency singleton --exclusive --gpus-per-node=8 --partition=batch_block1,batch_block3,batch_block4
...
NAME="ablation-eloirm-remax-hh-lr1e-6-bsz64-kl0.01"
COMMIT_ID=00c8a34
...
&& python -u examples/nlp/gpt/train_gpt_reinforce_actor.py \
    ...
    trainer.reinforce.baseline=\"ReMax\"
EOF

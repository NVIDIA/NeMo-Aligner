#!/bin/bash
#SBATCH -N 8 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_pg:42-9_critic+reward -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
#SBATCH hetjob
#SBATCH -N 2 --ntasks-per-node 8 -A llmservice_modelalignment_ppo --job-name llmservice_modelalignment_pg:42-9_actor+init_policy -t 4:00:00 --exclusive --dependency singleton --gpus-per-node=8 --partition=batch_block1
...
NAME="ablation-eloirm-vanilla-hh-lr1e-6-bsz64-kl0.01"
COMMIT_ID=770873d

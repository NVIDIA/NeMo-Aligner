...
def test_verify_is_valid_and_clamp_range(dummy_gpt_model):
    ...
    response_tokens = torch.nn.utils.rnn.pad_sequence([torch.tensor(x) for x in response_tokens], batch_first=True, padding_value=eos_id)

    context_lengths = torch.tensor([1, 2, 1, 2, 1])
    generation_lengths = torch.tensor([0, 1, len(extra_id_1_ids), len(extra_id_2_ids), len(extra_id_2_ids)]) + len(random_gen)

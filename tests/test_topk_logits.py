...
def loss_func(
    logits, target_logits, loss_mask, kd_loss="bwd_kl", logits_scale=1.0, target_logits_scale=1.0,
):
    ...
    return torch.sum(loss * loss_mask) / torch.sum(loss_mask).clamp(min=1.0)

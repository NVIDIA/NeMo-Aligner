...
@pytest.fixture
def make_actor_gpt_model():
    def _create_model(model_args: dict, trainer_args: dict):
        model_cfg = {
        ...
        cfg = DictConfig({"trainer": trainer_cfg, "model": model_cfg,})

        overrides = DictConfig({"trainer": trainer_args, "model": model_args,})
...
@pytest.fixture
def dummy_actor_gpt_model_with_pp(make_actor_gpt_model):

    yield make_actor_gpt_model(
        model_args={"pipeline_model_parallel_size": int(os.environ.get("PP", 2)),},
        trainer_args={"devices": int(os.environ.get("PP", 2))},
    )


@pytest.fixture
def dummy_actor_gpt_model_with_pp_cg(make_actor_gpt_model):
    yield make_actor_gpt_model(
        model_args={

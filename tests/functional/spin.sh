...
mpirun -np 2 --allow-run-as-root python -u ${GPFS}/examples/nlp/gpt/train_gpt_spin.py \
    ...
    ++trainer.spin.max_steps=-1 \
    ++trainer.spin.val_check_interval=4 \
    ++trainer.spin.save_interval=0 \
    ++trainer.spin.limit_val_batches=8 \
    ++trainer.spin.limit_train_batches=4 \
    ++model.optim.lr=4e-7 \
    ...
    ++model.spin.ref_policy_kl_penalty=[1.0,2.5,5.0] \
    ...
    ++trainer.spin.trt_llm.enable=false \
    ++trainer.spin.trt_llm.model_type=llama \

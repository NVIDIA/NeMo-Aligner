...
record = {'conversations': [{'from': 'User', 'value': 'how are you?'}, {'from': 'Assistant', 'value': 'I am fine, thank you.'}, {'from': 'User', 'value': 'why the sky is blue?'}, {'from': 'Assistant', 'value': ''}]}
...
for i in range(50):
    ...
    # 
    with ModelClient("localhost:2323", "search") as client:
        result_dict = client.infer_batch(action=np.array(best_actions).reshape(batch_size, -1).astype(np.int32), context_ids=encoded_context_data, parameters={"session": "test"})

...
from nemo_aligner.algorithms.self_taught import SelfTaughtTrainer, self_taught_custom_collate
...
"""Script to start Self-Taught training"""
...
@hydra_runner(config_path="conf", config_name="gpt_self_taught")
def main(cfg) -> None:
    ...
    trainer = resolve_and_create_trainer(cfg, "self_taught")
    ...
    if (not hasattr(ptl_model, "ref_policy_state_dict")) or ptl_model.ref_policy_state_dict is None:
    ...
    if cfg.model.data.get("sample", False):
        ...
        if cfg.trainer.self_taught.max_steps < 0:
            num_samples = None
        else:
            num_samples = cfg.trainer.self_taught.max_steps * cfg.model.global_batch_size
    else:
        num_samples = None
    ...
    if cfg.model.data.get("sample", False):
        num_samples = cfg.trainer.self_taught.limit_val_batches * val_data_cfg.global_batch_size
    else:
        num_samples = None
    ...
    self_taught_trainer = SelfTaughtTrainer(
        cfg=cfg.trainer.self_taught,
        model=ptl_model,
        optimizer=optimizer,
        scheduler=scheduler,
    ...
    if custom_trainer_state_dict is not None:
        self_taught_trainer.load_state_dict(custom_trainer_state_dict)

    self_taught_trainer.fit()

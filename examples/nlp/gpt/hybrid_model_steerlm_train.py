...
def main(cfg) -> None:
    ...
    all_data_ids = [item["data_id"] for item in train_data["policies"]]
    # split all_data_ids into 90% train and 10% val
    val_ids = set(all_data_ids[: len(all_data_ids) // 10])
    ...
    num_samples = compute_limit_batches(
        (len(all_data_ids) - len(val_ids)) // (cfg.model.inference.micro_batch_size * dp_size),
        cfg.trainer.deep_search.limit_val_batches,
    )
    ...
    deep_search_trainer = DeepSearchTrainer(
        ...
        val_dataloader_builder_func=None,
        train_dataloader_builder_func=None,
        feedback=feedback,
        logger=logger,

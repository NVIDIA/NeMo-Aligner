...
for split in ["train", "validation"]:
    ...
    policies = list(itertools.chain.from_iterable(x[split]["policies"] for x in loaded_files))
    values = list(itertools.chain.from_iterable(x[split]["values"] for x in loaded_files))

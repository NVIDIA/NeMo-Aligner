...
class ValueDataset:
    ...
    def __post_init__(self):
        ...
        if Path(self.path_to_jsonl).is_dir():
            data_files = list(map(str, sorted(Path(self.path_to_jsonl).glob("*.jsonl"))))
        else:
            data_files = [self.path_to_jsonl]

        self.data = load_dataset("json", data_files=data_files, cache_dir=self.cache_dir, num_proc=None)["train"]
